...
public class Base64 {
  ...
  public static byte[] decode(String data) {
    ...
    for (int i = 0; i < bytes.length; ) {
      int b;
      if (DECODE_TABLE[bytes[i]] != -1) {
        b = (DECODE_TABLE[bytes[i]] & MASK_8_BITS) << SHIFT_18;
      }
      ...
      int num = 0;
      if (i + 1 < bytes.length && DECODE_TABLE[bytes[i + 1]] != -1) {
        b = b | ((DECODE_TABLE[bytes[i + 1]] & MASK_8_BITS) << SHIFT_12);
        num++;
      }
      if (i + 2 < bytes.length && DECODE_TABLE[bytes[i + 2]] != -1) {
        b = b | ((DECODE_TABLE[bytes[i + 2]] & MASK_8_BITS) << SHIFT_6);
        num++;
      }
      if (i + 3 < bytes.length && DECODE_TABLE[bytes[i + 3]] != -1) {
        b = b | (DECODE_TABLE[bytes[i + 3]] & MASK_8_BITS);
        num++;
      }
