...
public class GetFollowedRequestTest extends AbstractDataTest<PagingCursorbased<Artist>> {
  private final GetFollowedArtistsRequest defaultRequest = SPOTIFY_API.getFollowedArtists(TYPE)
    .setHttpManager(
      TestUtil.MockedHttpManager.returningJson(
        "requests/data/users/GetFollowedRequest.json"))
    .after(ID_ARTIST)
    .limit(LIMIT)
    .build();

  private final GetFollowedArtistsRequest emptyRequest = SPOTIFY_API.getFollowedArtists(TYPE)
    .setHttpManager(
      TestUtil.MockedHttpManager.returningJson(
        "requests/data/users/GetFollowedRequest_None.json"))
    .after(ID_ARTIST)
    .limit(LIMIT)
    .build();

  public GetFollowedRequestTest() throws Exception {
  }
